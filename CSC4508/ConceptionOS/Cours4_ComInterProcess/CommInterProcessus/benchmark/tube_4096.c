...
/* Correspond au sujet de l'exercice 3 du TP noté de 2009 */
...
{
  if (write(distr_traite[1], req, TAILLE_MAX_REQUETE) < TAILLE_MAX_REQUETE){
    perror("write");
    exit(EXIT_FAILURE);
  }
}
  ...
  while (1)
    {
      ...
      if (read(distr_traite[0], req, TAILLE_MAX_REQUETE) < TAILLE_MAX_REQUETE){
	perror("read");
	exit(EXIT_FAILURE);
      }
